...
RSpec.describe "API_V1::Reservations", :type => :request do
  ...
  before do
    ...
    @reservation = Reservation.create!( :train_id => @train1.id, :seat_number => "1A", :customer_name => "foo", :customer_phone => "12345678" )
  end
  ...
  example "GET /api/v1/reservations/{booking_code}" do
    ...
    expect(response.body).to eq( { :booking_code => @reservation.booking_code,     :train_number => @reservation.train.number,     :train => {
